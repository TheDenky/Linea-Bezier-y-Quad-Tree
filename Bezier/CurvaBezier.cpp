...
//Clase de puntos para tomar los puntos
class Point {
public:
    float x, y;
    ...
    //operador overloading para signo '=' 
...
//Calcula los puntos de Bezier [generalized]
...
void myMouse(int button, int state, int x, int y) {
  // Si el botón izquierdo es clickeado
  if(button == GLUT_LEFT_BUTTON && state == GLUT_DOWN) {
    // Almacena cuando el mouse fue clickeado, Y es backwards.
    ...
    // si (click-amout) los puntos se dibujan haciendo la curva.
    if(points == clicks) 
    {
        glColor3f(0.2,1.0,0.0);
        // Dibujando las lineas de control
        for(int k=0;k<clicks-1;k++)
            drawLine(abc[k], abc[k+1]);

        Point p1 = abc[0];
        /* Dibuje cada segmento de la curva. Haga un incremento de t en cantidades más pequeñas para obtener una curva más detallada.*/
